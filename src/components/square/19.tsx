...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    if (index >= 4 && index <= 8) {
      keys.push(cid('left', index - 3, 0));
      return keys;
    }
    // Right side 14-19 (rows 1..6)
    if (index >= 9 && index <= 13) {
      keys.push(cid('right', index - 8, 5));
      return keys;
    }
    // Bottom row 18-25 (row 9, cols 0..7)
    if (index >= 14 && index <= 18) {
      keys.push(cid('bottom', 6, index - 13));
      return keys;
    }
    return keys;
  };
      ...
      >
        ...
        {Array.from({ length: 5 }, (_, rowIndex) => (
          <React.Fragment key={`middle-row-${rowIndex}`}>
            ...
            {rowIndex === 0 && (
              <div
                className="col-span-4 row-span-5 grid-cell active:animate-grid-pulse flex items-center justify-center text-white font-bold text-lg relative overflow-hidden"
                style={(() => {
            ...
            <div
              className="grid-cell active:animate-grid-pulse relative overflow-hidden cursor-pointer"
              style={getCellStyleWithFallback(cid('right', rowIndex + 1, 5))}
        ...
        <div className="col-span-6">
          ...
          >
            {Array.from({ length: 5 }, (_, colIndex) => {
              const key = cid('bottom', 6, colIndex + 1);
