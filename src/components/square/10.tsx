...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    if (index >= 5 && index <= 6) {
      keys.push(cid('right', index - 4, 7));
      return keys;
    }
    // Bottom row 18-25 (row 9, cols 0..7)
    if (index >= 7 && index <= 9) {
      keys.push(cid('bottom', 9, index - 6));
      return keys;
    }
    return keys;
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-2 md:p-6">
      ...
      <div
        className="grid grid-cols-5 bg-white rounded-xl shadow-2xl p-1 md:p-3"
        style={{
          gap: 'var(--gap)',
          ...
          gridTemplateColumns: 'repeat(5, var(--cell))'
        } as React.CSSProperties}
      >
        ...
        {Array.from({ length: 2 }, (_, rowIndex) => (
          <React.Fragment key={`middle-row-${rowIndex}`}>
            ...
            {rowIndex === 0 && (
              <div
                className="col-span-3 row-span-3 grid-cell active:animate-grid-pulse flex items-center justify-center text-white font-bold text-lg relative overflow-hidden"
                style={(() => {
