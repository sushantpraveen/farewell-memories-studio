...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    if (index >= 3 && index <= 5) {
      keys.push(cid('left', index - 2, 0));
      return keys;
    }
    // Right side 14-19 (rows 1..6)
    if (index >= 6 && index <= 8) {
      keys.push(cid('right', index - 5, 7));
      return keys;
    }
    // Bottom row 18-25 (row 9, cols 0..7)
    if (index >= 9 && index <= 12) {
      keys.push(cid('bottom', 5, index - 8));
      return keys;
    }
        ...
        {Array.from({ length: 3 }, (_, rowIndex) => (
          <React.Fragment key={`middle-row-${rowIndex}`}>
            ...
            <div
              className="grid-cell active:animate-grid-pulse relative overflow-hidden cursor-pointer"
              style={{
                ...(getCellStyleWithFallback(cid('left', rowIndex + 1, 0)) as any),
            ...
            <div
              ...
              style={{
                ...(getCellStyleWithFallback(cid('right', rowIndex + 1, 7)) as any),
                transform: 'translateY(calc(var(--row) / 2))',
              } as React.CSSProperties}
