...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    if (index >= 10 && index <= 13) {
      keys.push(cid('right', index - 9, 5));
      return keys;
    }
    // Bottom row 18-25 (row 9, cols 0..7)
    if (index >= 14 && index <= 19) {
      keys.push(cid('bottom', 5, index - 14));
      return keys;
    }
    return keys;
  };
  ...
  const buildAndDownload = async () => {
    ...
    try {
      await downloadImage('template-33.png', {
        cols: 6,
        rows: 6, // Increased to include bottom row and extension row
        ...
        draw: async ({ drawKey, ctx, width, height }) => {
          ...
          // 1. Draw top row (8 cells)
          for (let c = 0; c < 6; c++) {
            await drawKey(cid('top', 0, c), 0, c);
          }

          // 2. Draw left side (6 cells)
          for (let r = 1; r <= 4; r++) {
            await drawKey(cid('left', r, 0), r, 0);
          }

          // 3. Draw center cell (spans 6x6)
          await drawKey(cid('center', 0, 0), 1, 1, 4, 4);

          // 4. Draw right side (6 cells)
          for (let r = 1; r <= 4; r++) {
            await drawKey(cid('right', r, 5), r, 5);
          }

          // 5. Draw bottom row (8 cells);
          for (let c = 0; c < 6; c++) {
            // Use consistent cell ID format as shown in the grid
            await drawKey(cid('bottom', 5, c), 5, c, 1, 1);
          }

        //   // 6. Draw bottom extension (5 cells centered)
        //   const extensionCells = 4;
        //   const startCol = 2; // Start at 1.5 to center 5 cells in 8 columns
        //   const extensionRow = 8;
          
        //   // Draw the 5 cells with equal gaps on both sides
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-2 md:p-6">
      ...
      <div
        className="grid grid-cols-6 bg-white rounded-xl shadow-2xl p-1 md:p-3"
        style={{
          gap: 'var(--gap)',
      ...
      >
        ...
        {Array.from({ length: 4 }, (_, rowIndex) => (
          <React.Fragment key={`middle-row-${rowIndex}`}>
            ...
            {rowIndex === 0 && (
              <div
                className="col-span-4 row-span-4 grid-cell active:animate-grid-pulse flex items-center justify-center text-white font-bold text-lg relative overflow-hidden"
            ...
            <div
              className="grid-cell active:animate-grid-pulse relative overflow-hidden cursor-pointer"
              style={{ ...(getCellStyle(cid('right', rowIndex + 1, 5)) as any) }}
              onClick={() => handleCellClick(cid('right', rowIndex + 1, 5))}
              role="button"
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  handleCellClick(cid('right', rowIndex + 1, 5));
                }
              }}
            >
              {!cellImages[cid('right', rowIndex + 1, 5)] && (
                <div className="absolute inset-0 flex items-center justify-center text-white text-xs font-medium opacity-70">
                  +
                </div>
              )}
        ...
        {/* Bottom row - 8 cells */}
        {Array.from({ length: 6 }, (_, colIndex) => {
          const cellKey = cid('bottom', 5, colIndex); // This matches our download cell ID
          return (
            <div
              key={cellKey}
              className="grid-cell active:animate-grid-pulse relative overflow-hidden cursor-pointer"
