...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getCellIndexFromKey = (cellKey: string) => {
    ...
    if (section === 'top') {
      ...
      console.log("bottom-extension mapping → row:", row, "col:", col, "index:", 25 + col);
      return 25 + col;
    }
    else if (section === 'top-extension') {
      console.log("top-extension mapping → row:", row, "col:", col, "index:", 27 + col);
      return 28 + col;
    }
    return -1;
  };
