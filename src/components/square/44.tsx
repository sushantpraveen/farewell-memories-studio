...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    // Top extension 34-44 (4 cells centered)
    if (index >= 36 && index <= 45) {
      const col = index - 36; // 0..3
  ...
  const getCellIndexFromKey = (cellKey: string) => {
    ...
    if (section === 'top') {
      ...
      console.log("topExt mapping → row:", row, "col:", col, "index:", 36 + (col - 2));
      return 36 + col;
    }
  ...
  const buildAndDownload = async () => {
    ...
    try {
      await downloadImage('template-44.png', {
        cols: 8,
