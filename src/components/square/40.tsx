...
const GridBoard: React.FC<GridBoardProps> = ({ previewMember, existingMembers = [], centerEmptyDefault = false }) => {
  ...
  const getKeysForIndex = (index: number): string[] => {
    ...
    // Bottom extension 28-33 (6 cells centered)
    ...
    // Top extension 34-40 (4 cells centered)
    if (index >= 34 && index <= 39) {
      const col = index - 34; // 0..3
  ...
  const getCellIndexFromKey = (cellKey: string) => {
    ...
    if (section === 'top') {
      ...
      console.log("topExt mapping → row:", row, "col:", col, "index:", 34 + (col - 2));
      return 34 + col;
    }
  ...
  const buildAndDownload = async () => {
    ...
    try {
      await downloadImage('template-40.png', {
        cols: 8,
